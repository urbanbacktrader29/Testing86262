...
export default function ReportPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await addReport(position.lat, position.lng, type, speedLimit ? Number(speedLimit) : undefined, note);
      setDone(true);
      setTimeout(() => navigate("/liste"), 900);
    } catch {
      setError("Meldung konnte nicht gespeichert werden. Bitte erneut versuchen.");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="max-w-md mx-auto space-y-4">
      <div>
        ...
        <p className="text-sm text-slate-400">
          Die Meldung wird an deiner aktuellen Position gespeichert und live an alle Nutzer:innen der App synchronisiert.
        </p>
      </div>
      ...
      {done ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {submitting ? "Wird gemeldet …" : "Blitzer melden"}
          </button>
        </form>
      )}
    </div>
