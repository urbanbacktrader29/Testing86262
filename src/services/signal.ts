...
// Gemini's stated confidence against.
 ...
 * to our Gemini-backed /api/signal endpoint for the actual directional
 * judgment, and combines that with ATR-based (not AI-guessed) entry/SL/TP
 * levels. Returns null if there isn't enough candle history yet.
 */
export async function fetchCoinSignal(listing: CoinListing): Promise<TradeSignal | null> {
  ...
  const ai = (await res.json()) as GeminiSignalResponse;
